...
\indent{The classification output is shown in Table \ref{table:metrics}. The worst classification result was received for the Black-grass class. This type of plant is difficult to segment on an equal basis with the Loose Silky-bent class (Fig. \ref{fig:seg_degradation}), for which result is significantly better. The Black-grass plants have purple roots, but the segmentation algorithm we use does not work properly in the purple colour values range. Another cause of the result is in the predominance of the Loose Silky-bent training data size.} \\
